 ...
 * @method     ChildSettingsQuery orderByValorInscricao($order = Criteria::ASC) Order by the valor_inscricao column
 *
 * @method     ChildSettingsQuery groupById() Group by the id column
 * @method     ChildSettingsQuery groupByValorInscricao() Group by the valor_inscricao column
 *
 ...
 * @method     ChildSettings findOneByValorInscricao(double $valor_inscricao) Return the first ChildSettings filtered by the valor_inscricao column *
 ...
 * @method     ChildSettings requireOneByValorInscricao(double $valor_inscricao) Return the first ChildSettings filtered by the valor_inscricao column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildSettings[]|ObjectCollection findByValorInscricao(double $valor_inscricao) Return ChildSettings objects filtered by the valor_inscricao column
...
abstract class SettingsQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'my_cine_mania', $modelName = '\\Settings', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = SettingsTableMap::getInstanceFromPool((string) $key))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT id, valor_inscricao FROM settings WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            SettingsTableMap::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
    ...
    /**
     * Filter the query on the valor_inscricao column
     *
     * Example usage:
     * <code>
     * $query->filterByValorInscricao(1234); // WHERE valor_inscricao = 1234
     * $query->filterByValorInscricao(array(12, 34)); // WHERE valor_inscricao IN (12, 34)
     * $query->filterByValorInscricao(array('min' => 12)); // WHERE valor_inscricao > 12
     * </code>
     *
     * @param     mixed $valorInscricao The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return $this|ChildSettingsQuery The current query, for fluid interface
     */
    public function filterByValorInscricao($valorInscricao = null, $comparison = null)
    {
        if (is_array($valorInscricao)) {
            $useMinMax = false;
            if (isset($valorInscricao['min'])) {
                $this->addUsingAlias(SettingsTableMap::COL_VALOR_INSCRICAO, $valorInscricao['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($valorInscricao['max'])) {
                $this->addUsingAlias(SettingsTableMap::COL_VALOR_INSCRICAO, $valorInscricao['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(SettingsTableMap::COL_VALOR_INSCRICAO, $valorInscricao, $comparison);
    }
